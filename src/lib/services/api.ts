export async function fetchWorks() {
    const response = await fetch(`${import.meta.env.VITE_BACKEND_FRASES}/api/works`);
    if (!response.ok) {
...
export async function fetchWorkById(idWork: string) {
    const response = await fetch(`${import.meta.env.VITE_BACKEND_FRASES}/api/works/${idWork}`);
    if (!response.ok) {
...
export async function saveQuote({ quote, idWork }: {
    ...
    const response = await fetch(`${import.meta.env.VITE_BACKEND_FRASES}/api/quotes/`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
